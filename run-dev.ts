...
const server: ChildProcess = spawn('node', ['./node_modules/ts-node/dist/bin.js', 'server.ts'], {
...
server.stdout?.on('data', (data: Buffer) => {
  ...
  if (output.includes('Server running on port') && !serverStarted) {
    ...
    setTimeout(() => {
      ...
      const indexPath: string = path.join(process.cwd(), 'index.html');
      
      // Open specifically in Chrome browser
      const fileUrl: string = `file:///${indexPath.replace(/\\/g, '/')}`;
